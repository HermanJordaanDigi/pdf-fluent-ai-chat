  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        float horizontalFade = 1.0 - (cos(uv.x * 6.283185) * 0.5 + 0.5);
        float verticalFade   = 1.0 - (cos(uv.y * 6.283185) * 0.5 + 0.5);

        space.y += random(space.x * warpFrequency + iTime * warpSpeed) * warpAmplitude * (0.5 + horizontalFade);
        space.x += random(space.y * warpFrequency + iTime * warpSpeed + 2.0) * warpAmplitude * horizontalFade;

        vec4 lines = vec4(0.0);
        vec4 bgColor1 = vec4(0.08, 0.08, 0.08, 1.0);
        ...
        for (int l = 0; l < linesPerGroup; l++) {
          ...
          float lineShape = drawSmoothLine(linePosition, halfWidth, space.y) / 2.0
          ...
          float line = lineShape + circle;

          vec4 gold  = vec4(1.00, 0.84, 0.00, 1.0);
          vec4 gray  = vec4(0.75, 0.75, 0.75, 1.0);
          vec4 white = vec4(1.00, 1.00, 1.00, 1.0);

          int choice = int(mod(float(l) * 3.0 + floor(rand * 3.0), 3.0));
          vec4 lineColor = (choice == 0) ? gold
                            : (choice == 1) ? gray
                                            : white;
