...
import { Upload, FileText, Lightbulb, MessageCircle, ArrowLeft, Send, Download, LogIn } from 'lucide-react';
      ...
      <div className="min-h-screen bg-[#F5F0E1] flex flex-col">
        ...
        <div className="bg-white shadow-sm border-b border-gray-200 p-4">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <Button variant="ghost" onClick={() => setChatMode(false)} className="text-[#333333] hover:bg-[#EEEEEE] p-2">
        ...
        <div className="flex-1 max-w-4xl mx-auto w-full p-4 overflow-y-auto">
          <div className="space-y-4">
            {chatMessages.map(message => <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl shadow-sm ${message.isUser ? 'bg-[#CCCCCC] text-[#333333]' : message.id === 'system' ? 'bg-[#EEEEEE] text-[#666666] text-center' : 'bg-white text-[#333333]'}`}>
                  {message.content}
                </div>
              </div>)}
          </div>
        ...
        <div className="bg-white border-t border-gray-200 p-4">
          <div className="max-w-4xl mx-auto flex gap-2">
            <Input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Ask a question about your document..." className="flex-1 border-[#CCCCCC] focus:border-[#AAAAAA]" onKeyPress={e => e.key === 'Enter' && handleSendMessage()} disabled={isSendingMessage} />
            <Button onClick={handleSendMessage} disabled={!chatInput.trim() || isSendingMessage} className="bg-[#AAAAAA] hover:bg-white hover:text-[#333333] border border-[#AAAAAA]">
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <div className="flex items-center space-x-2">
                <Switch id="summary" checked={generateSummary} onCheckedChange={setGenerateSummary} className="data-[state=checked]:bg-[#DDDDDD]" />
              ...
              <div className="flex items-center space-x-2">
                <Switch id="insights" checked={generateInsights} onCheckedChange={setGenerateInsights} className="data-[state=checked]:bg-[#DDDDDD]" />
            ...
            {!generateSummary && !generateInsights && <p className="text-[#777777] text-sm text-center mt-4">
        ...
        {translatedDoc && (
          <div className="space-y-6">
            ...
            <Card className="shadow-lg border-0">
              ...
              <CardContent>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <Button onClick={handleDownload} className="bg-[#AAAAAA] hover:bg-white hover:text-[#333333] border border-[#AAAAAA]">
            ...
            {/* Summary Section */}
            {generateSummary && <Card className="shadow-lg border-0">
                <CardHeader>
                  <CardTitle className="text-lg text-[#333333]">Summary</CardTitle>
                </CardHeader>
                <CardContent>
                  {isProcessingSummary ? <div className="bg-[#EEEEEE] animate-pulse rounded-lg h-32"></div> : summary ? <div className="bg-[#FAFAFA] border border-[#CCCCCC] rounded-lg p-4 max-h-48 overflow-y-auto">
            ...
            {generateInsights && <Card className="shadow-lg border-0">
                <CardHeader>
                  <CardTitle className="text-lg text-[#333333]">5 Key Insights</CardTitle>
                </CardHeader>
                <CardContent>
                  {isProcessingInsights ? <div className="space-y-3">
                      {[...Array(5)].map((_, i) => <div key={i} className="bg-[#EEEEEE] animate-pulse rounded h-8"></div>)}
                    </div> : insights.length > 0 ? <ul className="space-y-3">
                      {insights.map((insight, index) => <li key={index} className="flex items-start gap-3">
